...
class EventWarping(nn.Module):
    ...
    def __init__(self, res, config):
        super(EventWarping, self).__init__()
        self.res = res
        self.flow_scaling = 1.0
        # self.flow_scaling = max(res)
        self.weight = config.get("flow_regul_weight", 1.0)
        # self.weight = 0.0
    ...
    def forward(self, flow_list, event_list, pol_mask):
        ...
        """
            ...
            # V
            # y
        event_list[: ,:, 0:1] = (1 - event_list[: ,:, 0:1])
        ...
        for flow in flow_list:
            ...
            # event_flowy = torch.gather(flow[:, 0, :], 1, flow_idx.long())  # vertical component
            # event_flowx = torch.gather(flow[:, 1, :], 1, flow_idx.long())  # horizontal component
